...
function initProductSlideshow() {
  ...
  prevBtn.addEventListener('click', () => {
    prevSlide();
    if (isPlaying) startSlideshow(); // Reset timer
  });
  ...
  // Pause on hover
  const container = document.querySelector('.product-slideshow-container');
...
// === Affichage des produits dynamiques (Full Screen Modal) ===
function showProduct(type) {
  ...
  details.innerHTML = `
    ...
    <div class="modal-content-wrapper">
      <h3 class="product-hero-title">${product.title.replace(/^[^\s]+\s/, '')}</h3> <!-- Remove emoji for cleaner look if needed, or keep -->
      <p class="product-hero-subtitle">${product.description}</p>
      
      <div class="product-actions">
        <a href="#contact" class="apple-btn apple-btn-primary" onclick="closeProductModal()">
          Commander
        </a>
        <a href="#contact" class="apple-btn apple-btn-secondary" onclick="closeProductModal()">
          En savoir plus
        </a>
      </div>
      ...
      <div class="product-gallery">
        ${product.images.map(img => `
          <div class="product-gallery-item fade-in">
